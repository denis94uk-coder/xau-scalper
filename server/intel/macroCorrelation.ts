...
async function fetchYahooQuote(
  ...
  try {
    ...
    const valid = closes.filter((value): value is number =>
      Number.isFinite(value),
    );
    const last = valid.length > 0 ? valid[valid.length - 1] : undefined;
    const price =
      Number.isFinite(metaPrice) && metaPrice > 0 ? metaPrice : last;
    if (!price || !Number.isFinite(price)) return null;
    const previous = valid.length > 1 ? valid[valid.length - 2] : undefined;
    const changePct =
      previous && previous !== 0 ? ((price - previous) / previous) * 100 : 0;
    const lastTimestamp =
...
async function fetchYahooMacroData(doFetch: Fetcher) {
  const [dxy, spx, us10y, paxg] = await Promise.all([
    fetchYahooQuote(doFetch, "DX-Y.NYB"),
    fetchYahooQuote(doFetch, "^GSPC"),
    fetchYahooQuote(doFetch, "^TNX"),
    fetchBinancePrice(doFetch, "PAXGUSDT"),
  ]);
  if (!dxy || !spx || !us10y || !paxg) {
    throw new Error(
      "Yahoo macro data incomplete; preserving the previous state",
    );
  }
  return {
    dxy,
...
export async function fetchMacroData(db: Db, fetcher?: Fetcher): Promise<void> {
  const doFetch = fetcher ?? fetch;
  try {
    const macroData = await fetchYahooMacroData(doFetch);
