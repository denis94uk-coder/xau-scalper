...
export function MacroCorrelation() {
  ...
  const assets: MacroAsset[] = [
    {
      label: "DXY", icon: "💵",
      price: macro.dxyPrice, change: macro.dxyChange,
      correlation: macro.dxyCorrelation, divergence: macro.dxyDivergence,
      divType: macro.dxyDivType, relationship: "Inverse"
    },
    {
      label: "US10Y", icon: "🏦",
      price: macro.us10yPrice, change: macro.us10yChange,
      correlation: macro.us10yCorrelation, divergence: macro.us10yDivergence,
      divType: macro.us10yDivType, relationship: "Inverse"
    },
    {
      label: "S&P 500", icon: "📊",
      price: macro.spxPrice, change: macro.spxChange,
      correlation: macro.spxCorrelation, divergence: macro.spxDivergence,
      divType: macro.spxDivType, relationship: "Variable"
    },
  ];

  const biasColor = macro.overallMacroBias === "BULLISH" ? "#00E676"
    : macro.overallMacroBias === "BEARISH" ? "#FF5252"
    : "#FFB74D";

  const biasIcon = macro.overallMacroBias === "BULLISH" ? "🟢"
    ...
    <div className="rounded-lg bg-secondary/20 border border-white/5 p-3">
      ...
      {assets.some(a => a.divergence) && (
        <div className="mt-2 pt-2 border-t border-white/5 space-y-1">
          {assets.filter(a => a.divergence).map((asset) => (
            <div key={`div-${asset.label}`}
                 className="flex items-center gap-2 text-[10px] rounded-md px-2 py-1"
                 style={{
                   backgroundColor: asset.divType === "BULLISH_GOLD" ? "rgba(0,230,118,0.08)" : "rgba(255,82,82,0.08)",
...
function AssetCard({ asset }: { asset: MacroAsset }) {
  const changeColor = asset.change > 0 ? "#00E676" : asset.change < 0 ? "#FF5252" : "#888";
