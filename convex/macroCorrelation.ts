...
async function fetchBinancePrice(symbol: string): Promise<{ price: number; change: number; changePct: number } | null> {
  try {
...
async function fetchMacroViaBinance() {
  ...
  return { dxyProxy, dxyChange, goldPrice: paxg?.price ?? 0, goldChange: paxg?.changePct ?? 0 };
}

async function fetchSPXProxy(): Promise<{ price: number; change: number } | null> {
  const btc = await fetchBinancePrice("BTCUSDT");
  if (!btc) return null;
  return { price: btc.price / 12, change: btc.changePct };
}

function calcCorrelation(goldChange: number, assetChange: number, expectedSign: number): number {
  if (goldChange === 0 || assetChange === 0) return 0;
  const sameDir = (goldChange > 0 && assetChange > 0) || (goldChange < 0 && assetChange < 0);
  const strength = Math.min(1, (Math.abs(goldChange) + Math.abs(assetChange)) / 4);
  return (sameDir ? strength : -strength) * expectedSign;
}

function detectDivergence(goldChange: number, assetChange: number, rel: "INVERSE" | "DIRECT") {
  const t = 0.3;
  if (Math.abs(goldChange) < t && Math.abs(assetChange) < t) return { alert: false, type: "NONE" };
  if (rel === "INVERSE") {
    if (assetChange < -t && goldChange < -t) return { alert: true, type: "BULLISH_GOLD" };
    if (assetChange > t && goldChange > t) return { alert: true, type: "BEARISH_GOLD" };
...
export const fetchMacroData = internalAction({
  ...
  handler: async (ctx) => {
    try {
      ...
      const dxyDiv = detectDivergence(goldChange, macroData.dxyChange, "INVERSE");
      ...
      let bull = 0, bear = 0;
      if (macroData.dxyChange < -0.2) bull++; if (macroData.dxyChange > 0.2) bear++;
      if (us10yChange < -0.1) bull++; if (us10yChange > 0.1) bear++;
      if (spxChange < -0.3) bull++; if (spxChange > 0.3) bear++;
      if (dxyDiv.alert && dxyDiv.type === "BULLISH_GOLD") bull += 2;
      if (dxyDiv.alert && dxyDiv.type === "BEARISH_GOLD") bear += 2;

      const overallMacroBias = bull > bear ? "BULLISH" : bear > bull ? "BEARISH" : "NEUTRAL";
      ...
      if (overallMacroBias === "BULLISH") {
        ...
        description = "Mixed macro signals — no clear directional bias from macro data.";
      }
      ...
      console.log(`[Macro] Bias: ${overallMacroBias} (${biasStrength}%) | DXY: ${macroData.dxyChange > 0 ? "+" : ""}${macroData.dxyChange.toFixed(2)}%`);
    } catch (e: any) {
